...
/* ----------------------------------------------------------------------------- *\
    name        : LoRa_transmit_IT

    description : Асинхронная передача данных через LoRa с использованием прерываний
    ...
    returns     : 1 в случае успешного запуска передачи, 0 в случае ошибки
\* ----------------------------------------------------------------------------- */
uint8_t LoRa_transmit_IT(LoRa* _LoRa, uint8_t* data, uint8_t length) {
    if (_LoRa->tx_in_progress) {
        return 0; // Передача уже выполняется
    }

    uint8_t read;

    // Сохраняем параметры передачи
    _LoRa->tx_buffer = data;
    _LoRa->tx_length = length;
    _LoRa->tx_in_progress = 1;
    ...
    // Записываем данные в FIFO (СИНХРОННО, т.к. это внутренняя операция)
    LoRa_BurstWrite(_LoRa, RegFiFo, data, length);

    // Переключаемся в режим передачи
    LoRa_gotoMode(_LoRa, TRANSMIT_MODE);

    // Данные уже в FIFO, теперь ждем прерывание DIO0
    return 1;
}

// ===== Исправленная функция LoRa_receive_IT =====
uint8_t LoRa_receive_IT(LoRa* _LoRa, uint8_t* buffer, uint8_t length) {
    if (_LoRa->rx_in_progress) {
        return 0; // Приём уже выполняется
    }

    _LoRa->rx_buffer = buffer;
    _LoRa->rx_length = length;
    _LoRa->rx_in_progress = 1;

    // Запускаем непрерывный приём
    LoRa_gotoMode(_LoRa, RXCONTIN_MODE);

    return 1;
}

// ===== Обработчик прерывания DIO0 =====
void LoRa_handle_dio0_interrupt(LoRa* _LoRa) {
    uint8_t irq_flags = LoRa_read(_LoRa, RegIrqFlags);

    // Обработка завершения передачи
    if ((irq_flags & 0x08) != 0) { // TxDone
        LoRa_write(_LoRa, RegIrqFlags, 0xFF); // Clear flags

        if (_LoRa->tx_in_progress) {
            _LoRa->tx_in_progress = 0;

            // Вызываем callback если установлен
            if (_LoRa->tx_complete_callback) {
                _LoRa->tx_complete_callback();
            }
        }
    }

    // Обработка завершения приёма
    if ((irq_flags & 0x40) != 0) { // RxDone
        LoRa_write(_LoRa, RegIrqFlags, 0xFF); // Clear flags

        if (_LoRa->rx_in_progress) {
            _LoRa->rx_in_progress = 0;

            // Читаем принятые данные
            uint8_t number_of_bytes = LoRa_read(_LoRa, RegRxNbBytes);
            uint8_t current_addr = LoRa_read(_LoRa, RegFiFoRxCurrentAddr);
            LoRa_write(_LoRa, RegFiFoAddPtr, current_addr);

            uint8_t min = _LoRa->rx_length >= number_of_bytes ? number_of_bytes : _LoRa->rx_length;

            // Читаем данные из FIFO
            for (int i = 0; i < min; i++) {
                _LoRa->rx_buffer[i] = LoRa_read(_LoRa, RegFiFo);
            }

            // Вызываем callback если установлен
            if (_LoRa->rx_complete_callback) {
                _LoRa->rx_complete_callback(_LoRa->rx_buffer, min);
            }
